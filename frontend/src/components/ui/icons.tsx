...
/**
 * Dashboard-shell icon set. The line icons are re-exported from
 * `@animateicons/react/lucide` (hover-animated, `prefers-reduced-motion` aware).
 * They render `<div><svg/></div>`, are sized via the `size` prop (not an
 * `h-* w-*` class), and tint from the surrounding `text-*` via `currentColor`.
 *
 * `ScissorsIcon` has no equivalent in that set, so it stays hand-rolled below —
 * matched to the same API (`size` prop, 2px stroke) but static.
 *
 * The `FilledIcon` glyphs (Material Symbols) are unchanged: the Stylist
 * Dashboard is designed with solid icons rather than the line set.
 */
...
/**
 * Common shape for icons passed around as values (nav configs). Covers both the
 * `@animateicons/react` components and the hand-rolled `ScissorsIcon`.
 */
...
type ScissorsIconProps = Omit<HTMLAttributes<HTMLSpanElement>, "color"> & {
  size?: number;
  color?: string;
  /** Accepted for API parity with the animated icons; this glyph is static. */
  isAnimated?: boolean;
};
...
export function WalletCardsIcon(props: SVGProps<SVGSVGElement>) {
  return (
...
export function CalendarAvailableIcon(props: SVGProps<SVGSVGElement>) {
  return (
...
export function CalendarFilledIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <FilledIcon viewBox="0 0 18 20" {...props}>
...
export function UsersFilledIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <FilledIcon viewBox="0 0 22 16" {...props}>
